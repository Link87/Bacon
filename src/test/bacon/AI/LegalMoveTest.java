...
public class LegalMoveTest {
    ...
    public void legalMoves(){
        ...
        evaluatedTiles1 = LegalMoves.legalMoves(Game.getGame().getCurrentState(), 1, MoveType.OVERRIDE);
        for(Tile t: evaluatedTiles1){
            assertTrue("Player 1: Illegal Override Move at: ("+ t.x + "," + t.y +")" ,legalTiles1.contains(t));
        }

        evaluatedTiles2 = LegalMoves.legalMoves(Game.getGame().getCurrentState(), 2, MoveType.OVERRIDE);
        for(Tile t: evaluatedTiles2){
            assertTrue("Player 2: Illegal Override Move at: ("+ t.x + "," + t.y +")" , legalTiles2.contains(t));
        }

        evaluatedTiles3 = LegalMoves.legalMoves(Game.getGame().getCurrentState(), 3, MoveType.OVERRIDE);
        for(Tile t: evaluatedTiles3){
        ...
        evaluatedTiles1 = LegalMoves.legalMoves(Game.getGame().getCurrentState(), 1, MoveType.BOMB);
        for(Tile t: evaluatedTiles1){
